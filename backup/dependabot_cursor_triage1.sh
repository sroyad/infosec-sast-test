...
# GitHub Actions may pass booleans as "true"/"True", JSON booleans, or with whitespace.
# Strict [ "$AUTO_DISMISS" = "true" ] alone caused auto-dismiss to stay off even when the UI was checked.
_trim_ad=$(echo "${AUTO_DISMISS}" | tr -d '\r\n' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
case "${_trim_ad}" in
    true|True|TRUE|1|yes|Yes|YES) AUTO_DISMISS="true" ;;
    *) AUTO_DISMISS="false" ;;
esac
...
main() {
    ...
    if [ "${AUTO_DISMISS}" = "true" ]; then
        log_info "Auto-dismiss is ON — false positives will be PATCH-dismissed on GitHub (token must allow it)."
    else
        log_info "Auto-dismiss is OFF — alerts will be classified only; set workflow input Auto-dismiss to dismiss on GitHub."
    fi
    echo ""
